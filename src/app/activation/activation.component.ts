...
export class ActivationComponent implements OnInit {
  ...
  public setAlert(alert:any){
    ...
    if(this.pmuid.length==0 || this.pmuid==null){
      this.pmuid="99999999"
    }
...
activateProfile(){
  ...
  updateStatus(status,msg){
   ...
   formData.append('type','customer');
        ...
        updatemsisdn(msisdn,token){
          console.log("Updatemsisdn",msisdn,token)
          ...
          this.http.post('https://ekyc.chili.mu:9443/updateMsisdn/wrong_msisdn_update.php',request,{ headers: headers}).subscribe((data:any)=>{
            let response = data;
            console.log(response);
            alert("MSISDN Updated Successfully")
